...
import { disconnectConnectedAccount, deleteConnectedAccount, getConnectedAccounts } from '../shared/utils/dbHelpers.js';

const router = Router();

const defaultAccountsDb = [
  { id: 'acc_1', platform: 'x', name: 'SocialFlow HQ', username: '@socialflow', followers: 14200, status: 'connected', avatar: 'https://images.unsplash.com/photo-1611605698335-8b1569810432?auto=format&fit=crop&w=150&q=80' },
...
router.get('/', (req, res) => {
  const dynamicAccounts = getConnectedAccounts();
  // Merge dynamic accounts with defaults
  const accountsMap = new Map();
  defaultAccountsDb.forEach(a => accountsMap.set(a.platform, a));
  dynamicAccounts.forEach(a => {
    accountsMap.set(a.platform, {
      id: a.id,
      platform: a.platform,
      name: a.name,
